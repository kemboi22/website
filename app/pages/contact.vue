<script setup lang="ts">
import { ref } from "vue";
import { useHead } from "#imports";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { Label } from "@/components/ui/label";
import { Card } from "@/components/ui/card";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
  AccordionTrigger,
} from "@/components/ui/accordion";

const form = ref({
  name: "",
  email: "",
  subject: "",
  message: "",
});

const isSubmitting = ref(false);
const submitMessage = ref("");
const submitSuccess = ref(false);
const openFaq = ref<number | null>(null);

const handleSubmit = async () => {
  isSubmitting.value = true;
  submitMessage.value = "";

  // Simulate form submission
  await new Promise((resolve) => setTimeout(resolve, 1500));

  // In a real application, you would send the form data to your backend here
  console.log("Form submitted:", form.value);

  submitSuccess.value = true;
  submitMessage.value =
    "Thank you for your message! I'll get back to you soon.";

  // Reset form
  form.value = {
    name: "",
    email: "",
    subject: "",
    message: "",
  };

  isSubmitting.value = false;

  // Clear success message after 5 seconds
  setTimeout(() => {
    submitMessage.value = "";
  }, 5000);
};

const faqs = [
  {
    question: "What is your typical project timeline?",
    answer:
      "Project timelines vary depending on scope and complexity. A simple website might take 2-4 weeks, while a complex web application could take 2-3 months. I provide detailed timelines during the initial consultation.",
  },
  {
    question: "Do you work with clients remotely?",
    answer:
      "Yes! I work with clients worldwide. I'm experienced in remote collaboration and use tools like Zoom, Slack, and Figma to ensure smooth communication throughout the project.",
  },
  {
    question: "What is your design process?",
    answer:
      "My process typically includes: discovery and research, wireframing and prototyping, visual design, development, testing, and launch. I involve clients at every stage to ensure the final product meets their vision.",
  },
  {
    question: "Do you provide ongoing support after launch?",
    answer:
      "Yes, I offer maintenance and support packages for all projects. This includes bug fixes, updates, and minor modifications. We can discuss the best support plan for your needs.",
  },
  {
    question: "What are your rates?",
    answer:
      "Rates vary depending on project scope, timeline, and complexity. I offer both project-based and hourly rates. Contact me with your project details for a custom quote.",
  },
];

useHead({
  title: "Contact - Portfolio",
  meta: [
    {
      name: "description",
      content: "Get in touch to discuss your project or just say hello.",
    },
  ],
});
</script>
<template>
  <div class="pt-16">
    <!-- Hero Section -->
    <section
      class="min-h-[50vh] flex items-center justify-center px-6 border-b border-border"
    >
      <div class="container mx-auto max-w-4xl text-center space-y-6">
        <span
          class="text-xs font-mono uppercase tracking-widest text-muted-foreground"
          >Get in Touch</span
        >
        <h1 class="text-5xl md:text-7xl font-serif font-bold tracking-tight">
          Let's Create
          <span class="block mt-2 italic">Something Great</span>
        </h1>
        <p
          class="text-lg text-muted-foreground max-w-2xl mx-auto leading-relaxed"
        >
          Whether you have a project in mind or just want to chat about design
          and development, I'd love to hear from you.
        </p>
      </div>
    </section>

    <!-- Contact Content -->
    <section class="py-16 md:py-24 px-4 md:px-6">
      <div class="container mx-auto max-w-6xl">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-16">
          <!-- Contact Form -->
          <div class="space-y-8">
            <div>
              <h2 class="text-3xl font-serif font-bold mb-4">Send a Message</h2>
              <p class="text-muted-foreground leading-relaxed">
                Fill out the form below and I'll get back to you as soon as
                possible.
              </p>
            </div>

            <form @submit.prevent="handleSubmit" class="space-y-6">
              <!-- Using shadcn-vue form components -->
              <div class="space-y-2">
                <Label for="name">Name *</Label>
                <Input
                  id="name"
                  v-model="form.name"
                  type="text"
                  required
                  placeholder="Your name"
                />
              </div>

              <div class="space-y-2">
                <Label for="email">Email *</Label>
                <Input
                  id="email"
                  v-model="form.email"
                  type="email"
                  required
                  placeholder="your@email.com"
                />
              </div>

              <div class="space-y-2">
                <Label for="subject">Subject *</Label>
                <Input
                  id="subject"
                  v-model="form.subject"
                  type="text"
                  required
                  placeholder="What's this about?"
                />
              </div>

              <div class="space-y-2">
                <Label for="message">Message *</Label>
                <Textarea
                  id="message"
                  v-model="form.message"
                  required
                  rows="6"
                  placeholder="Tell me about your project..."
                />
              </div>

              <!-- Using Button component -->
              <Button
                type="submit"
                :disabled="isSubmitting"
                class="w-full uppercase tracking-wider"
                size="lg"
              >
                {{ isSubmitting ? "Sending..." : "Send Message" }}
              </Button>

              <p
                v-if="submitMessage"
                :class="[
                  'text-sm text-center',
                  submitSuccess ? 'text-foreground' : 'text-muted-foreground',
                ]"
              >
                {{ submitMessage }}
              </p>
            </form>
          </div>

          <!-- Contact Info -->
          <div class="space-y-12">
            <div>
              <h2 class="text-2xl md:text-3xl font-serif font-bold mb-4">
                Other Ways to Connect
              </h2>
              <p class="text-muted-foreground leading-relaxed">
                Prefer a different method? Feel free to reach out through any of
                these channels.
              </p>
            </div>

            <!-- Contact Methods -->
            <div class="space-y-8">
              <!-- Using Card component -->
              <Card
                class="flex items-start gap-4 p-6 hover:border-foreground transition-colors group"
              >
                <div
                  class="w-12 h-12 flex items-center justify-center border border-border group-hover:border-foreground flex-shrink-0 transition-colors"
                >
                  <svg
                    class="w-5 h-5"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      stroke-width="2"
                      d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"
                    />
                  </svg>
                </div>
                <div>
                  <h3 class="font-serif font-bold text-lg mb-1">Email</h3>
                  <a
                    href="mailto:kemboielvis22@gmail.com"
                    class="text-muted-foreground hover:text-foreground transition-colors break-all"
                  >
                    kemboielvis22@gmail.com
                  </a>
                </div>
              </Card>

              <Card
                class="flex items-start gap-4 p-6 hover:border-foreground transition-colors group"
              >
                <div
                  class="w-12 h-12 flex items-center justify-center border border-border group-hover:border-foreground flex-shrink-0 transition-colors"
                >
                  <svg
                    class="w-5 h-5"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      stroke-width="2"
                      d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"
                    />
                  </svg>
                </div>
                <div>
                  <h3 class="font-serif font-bold text-lg mb-1">Phone</h3>
                  <a
                    href="tel:+254718240819"
                    class="text-muted-foreground hover:text-foreground transition-colors"
                  >
                    +254 718 240 819
                  </a>
                </div>
              </Card>

              <Card
                class="flex items-start gap-4 p-6 hover:border-foreground transition-colors group"
              >
                <div
                  class="w-12 h-12 flex items-center justify-center border border-border group-hover:border-foreground flex-shrink-0 transition-colors"
                >
                  <svg
                    class="w-5 h-5"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      stroke-width="2"
                      d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.828 0l-4.244-4.243a8 8 0 1111.314 0z"
                    />
                    <path
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      stroke-width="2"
                      d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"
                    />
                  </svg>
                </div>
                <div>
                  <h3 class="font-serif font-bold text-lg mb-1">Location</h3>
                  <p class="text-muted-foreground">
                    Nairobi, Kenya<br />
                    Available for remote work
                  </p>
                </div>
              </Card>

              <Card
                class="flex items-start gap-4 p-6 hover:border-foreground transition-colors group"
              >
                <div
                  class="w-12 h-12 flex items-center justify-center border border-border group-hover:border-foreground flex-shrink-0 transition-colors"
                >
                  <svg
                    class="w-5 h-5"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      stroke-width="2"
                      d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"
                    />
                  </svg>
                </div>
                <div>
                  <h3 class="font-serif font-bold text-lg mb-1">
                    Availability
                  </h3>
                  <p class="text-muted-foreground">
                    Currently accepting new projects<br />
                    Response time: 24-48 hours
                  </p>
                </div>
              </Card>
            </div>

            <!-- Social Links -->
            <div class="pt-8 border-t border-border">
              <h3
                class="text-sm font-mono uppercase tracking-widest text-muted-foreground mb-6"
              >
                Follow Me
              </h3>
              <div class="flex flex-wrap gap-4">
                <Button
                  as-child
                  variant="outline"
                  size="icon"
                  class="w-12 h-12"
                >
                  <a
                    href="https://github.com/kemboielvis22"
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label="GitHub"
                  >
                    <svg
                      class="w-5 h-5"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"
                      />
                    </svg>
                  </a>
                </Button>
                <Button
                  as-child
                  variant="outline"
                  size="icon"
                  class="w-12 h-12"
                >
                  <a
                    href="https://linkedin.com/in/kemboi-elvis-ab7a431b2"
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label="LinkedIn"
                  >
                    <svg
                      class="w-5 h-5"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"
                      />
                    </svg>
                  </a>
                </Button>
                <Button
                  as-child
                  variant="outline"
                  size="icon"
                  class="w-12 h-12"
                >
                  <a
                    href="https://twitter.com/kemboi22"
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label="Twitter"
                  >
                    <svg
                      class="w-5 h-5"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"
                      />
                    </svg>
                  </a>
                </Button>
                <Button
                  as-child
                  variant="outline"
                  size="icon"
                  class="w-12 h-12"
                >
                  <a
                    href="https://wa.me/254718240819"
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label="WhatsApp"
                  >
                    <svg
                      class="w-5 h-5"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"
                      />
                    </svg>
                  </a>
                </Button>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>

    <!-- FAQ Section -->
    <section class="py-24 px-6 bg-muted/30">
      <div class="container mx-auto max-w-4xl">
        <div class="text-center mb-16">
          <h2 class="text-3xl md:text-4xl font-serif font-bold mb-4">
            Frequently Asked Questions
          </h2>
          <p class="text-muted-foreground">Quick answers to common questions</p>
        </div>

        <!-- Using Accordion component from shadcn-vue -->
        <Accordion type="single" collapsible class="space-y-4">
          <AccordionItem
            v-for="(faq, index) in faqs"
            :key="index"
            :value="`item-${index}`"
            class="border border-border"
          >
            <AccordionTrigger class="px-6 py-4 hover:bg-muted/30 text-left">
              <h3 class="font-serif font-bold text-lg">{{ faq.question }}</h3>
            </AccordionTrigger>
            <AccordionContent class="px-6 pb-6">
              <p class="text-muted-foreground leading-relaxed">
                {{ faq.answer }}
              </p>
            </AccordionContent>
          </AccordionItem>
        </Accordion>
      </div>
    </section>
  </div>
</template>
