...
const {
  ...
  error: statsError,
} = useFetch<UserStats>("/api/github/stats");
const {
  data: pullRequests,
  pending: pullRequestsPending,
  error: pullRequestsError,
} = useFetch<UserPullRequest[]>("/api/github/pull-requests");
const {
  data: repositories,
  pending: repositoriesPending,
  error: repositoriesError,
} = useFetch<UserRepo[]>("/api/github/repositories");
const {
  data: packages,
  pending: packagesPending,
  error: packagesError,
} = useFetch<GitHubPackage[]>("/api/github/packages");
...
useHead({
  ...
  meta: [{ name: "description", content: `GitHub activity for ${GITHUB_USERNAME}` }],
});
</script>
...
<template>
  <div class="pt-16">
    <!-- Hero Section -->
    <section
      class="min-h-[50vh] flex items-center justify-center px-4 md:px-6 border-b border-border"
    >
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          <template v-if="statsPending">
            <div
              v-for="i in 4"
              :key="i"
              class="text-center space-y-2 p-4 md:p-6 border border-border animate-pulse"
            >
          ...
          <template v-else-if="statsError">
            <div
              class="col-span-full text-center text-destructive-foreground p-4 bg-destructive rounded"
            >
              Error loading stats.
            </div>
          </template>
          <template v-else>
            <div
              ...
              :key="stat.label"
              class="text-center space-y-2 p-4 md:p-6 border border-border hover:border-foreground transition-colors"
            >
              <div class="text-3xl md:text-4xl font-serif font-bold">
                {{ stat.value.toLocaleString() }}
              </div>
              <div class="text-xs font-mono uppercase tracking-widest text-muted-foreground">
                {{ stat.label }}
              </div>
            </div>
          </template>
    ...
    <Teleport to="body">
      <Transition name="fade">
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div
            class="relative bg-background border border-border w-full max-w-lg md:max-w-2xl max-h-[85vh] overflow-hidden flex flex-col"
          >
            <!-- Header -->
            <div class="flex items-start justify-between p-4 md:p-6 border-b border-border">
              <div>
                ...
                <div
                  class="flex flex-wrap items-center gap-2 mt-2 text-xs md:text-sm text-muted-foreground"
                >
                  <span class="font-mono uppercase px-2 py-1 border border-border">{{
                    selectedPkg?.package_type
                  }}</span>
                  <span>{{ selectedPkg?.visibility }}</span>
                  <span v-if="selectedPkg?.updated_at"
                    >Updated {{ new Date(selectedPkg.updated_at).toLocaleDateString() }}</span
                  >
                </div>
              </div>
                ...
                <div class="space-y-3">
                  ...
                  >
                    <div class="flex items-center gap-3">
                      <span
                        class="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-xs font-mono"
                        >{{ index + 1 }}</span
                      >
                      <div>
                        <div class="font-mono font-bold text-sm md:text-base">
                          {{ version.name }}
                        </div>
                        <div class="text-xs text-muted-foreground">
    ...
    <section class="py-16 md:py-24 px-4 md:px-6">
      <div class="container mx-auto max-w-6xl space-y-8 md:space-y-12">
        <div class="flex items-center justify-between">
          ...
          <span
            class="text-xs font-mono uppercase tracking-widest text-muted-foreground hidden md:block"
            >Last 30 Days</span
          >
        </div>
        ...
        <template v-else-if="pullRequestsError">
          <div class="text-center text-destructive-foreground p-4 bg-destructive rounded">
        ...
        <template v-else>
          <div class="space-y-4">
            <div
              ...
              :key="pr.id"
              class="border border-border hover:border-foreground p-4 md:p-6 transition-all"
            >
              <div class="flex items-start gap-4">
                <div
                  class="w-8 h-8 border border-border flex items-center justify-center flex-shrink-0"
                  :class="pr.status === 'merged' ? 'bg-foreground text-background' : ''"
                >
                  <svg
                ...
                <div class="flex-1 min-w-0">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    class="block text-lg font-bold hover:text-muted-foreground transition-colors truncate"
                    >{{ pr.title }}</a
                  >
        ...
        <template v-if="repositoriesPending">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            <div
              v-for="i in 2"
              :key="i"
              class="border border-border p-4 md:p-6 space-y-4 animate-pulse"
            >
              <div class="h-6 bg-muted rounded w-3/4"></div>
        ...
        <template v-else>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              class="border border-border hover:border-foreground p-4 md:p-6 space-y-4 transition-all"
            >
              <h3 class="text-lg font-bold group-hover:text-muted-foreground transition-colors">
                {{ repo.name }}
              </h3>
              <p class="text-sm text-muted-foreground line-clamp-2">{{ repo.description }}</p>
              <div
                class="flex flex-wrap items-center gap-4 text-xs font-mono text-muted-foreground"
              >
                <div class="flex items-center gap-2">
                  <div
                    class="w-3 h-3 rounded-full"
                    :style="{ backgroundColor: repo.languageColor || '#cccccc' }"
                  ></div>
